...
export class SubjectsService {
  ...
  async bulkImport(fileBuffer: Buffer) {
    ...
    const expectedHeaders = ['subject_code', 'subject_name', 'program_id', 'year_level', 'semester', 'category'];
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      if (subjectData.subject_name) {
        const subject = this.subjectsRepository.create(subjectData) as unknown as Subject;
    ...
    return { message: `Successfully imported ${importedSubjects.length} subjects` };
  }
  ...
  async linkSkill(subjectId: number, skillId: number, weight: number) {
    const subject = await this.subjectsRepository.findOne({ where: { subject_id: subjectId } });
    if (!subject) throw new NotFoundException('Subject not found');

    const skill = await this.skillsRepository.findOne({ where: { skill_id: skillId } });
    if (!skill) throw new NotFoundException('Skill not found');

    let link = await this.subjectSkillsRepository.findOne({ where: { subject_id: subjectId, skill_id: skillId } });
    if (link) {
      link.weight = weight;
    } else {
      link = this.subjectSkillsRepository.create({ subject_id: subjectId, skill_id: skillId, weight });
    }
    return this.subjectSkillsRepository.save(link);
  }

  async unlinkSkill(subjectId: number, skillId: number) {
    await this.subjectSkillsRepository.delete({ subject_id: subjectId, skill_id: skillId });
